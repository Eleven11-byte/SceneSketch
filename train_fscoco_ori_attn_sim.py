...
from utils.utils_train import print_trainable_params, get_similarity_map, zero_clapping, tensor_to_binary_img, sketch_text_pairs, get_threshold,\
    get_train_classes_with_image, sketch_text_image_pairs, save_checkpoint, load_checkpoint, get_train_classes, get_attention_map, zero_clapping_attn, get_attention_map_batch
...
def main(configs):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (sketches, captions) in enumerate(pbar):
            ...
            attn_maps = get_attention_map_batch(attn, image_size=sketches_w.shape[2:], patch_size=model.patch_size, B=sketches_w.shape[0], num_heads=12)

            threshold_value = get_threshold(learnable_threshold)

            fused = similarity_maps * attn_maps

            # weights_hard_sm = zero_clapping_attn(attn_maps, threshold_value)
            weights_hard_sm = zero_clapping(fused, threshold_value)
